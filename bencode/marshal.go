...
func unmarshalDict(p reflect.Value, dict map[string]*BObject) error {
	...
	for i, n := 0, v.NumField(); i < n; i++ {
		...
		case BLIST:
			// fv is rawFile field
			// ft is the Type of rawFile field.
			// fo is BObject
			if ft.Type.Kind() != reflect.Slice {
				break
			}
			list, _ := fo.List()
			lp := reflect.New(ft.Type)                             // Ptr, Type is rawFile field
			ls := reflect.MakeSlice(ft.Type, len(list), len(list)) // slice []ft.Type
			lp.Elem().Set(ls)
